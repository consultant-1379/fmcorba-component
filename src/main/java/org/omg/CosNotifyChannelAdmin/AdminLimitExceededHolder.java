package org.omg.CosNotifyChannelAdmin;

/**
 * <ul>
 * <li> <b>IDL Source</b>    "CosNotifyChannelAdmin.idl"
 * <li> <b>IDL Name</b>      ::CosNotifyChannelAdmin::AdminLimitExceeded
 * <li> <b>Repository Id</b> IDL:omg.org/CosNotifyChannelAdmin/AdminLimitExceeded:1.0
 * </ul>
 * <b>IDL definition:</b>
 * <pre>
 * exception AdminLimitExceeded {
  ...
};
 * </pre>
 */
public final class AdminLimitExceededHolder implements org.omg.CORBA.portable.Streamable {
public org.omg.CosNotifyChannelAdmin.AdminLimitExceeded value;

public AdminLimitExceededHolder () {
}

public AdminLimitExceededHolder (final org.omg.CosNotifyChannelAdmin.AdminLimitExceeded _vis_value) {
  this.value = _vis_value;
}

public void _read (final org.omg.CORBA.portable.InputStream input) {
  value = org.omg.CosNotifyChannelAdmin.AdminLimitExceededHelper.read(input);
}

public void _write (final org.omg.CORBA.portable.OutputStream output) {
  org.omg.CosNotifyChannelAdmin.AdminLimitExceededHelper.write(output, value);
}

public org.omg.CORBA.TypeCode _type () {
  return org.omg.CosNotifyChannelAdmin.AdminLimitExceededHelper.type();
}
}
