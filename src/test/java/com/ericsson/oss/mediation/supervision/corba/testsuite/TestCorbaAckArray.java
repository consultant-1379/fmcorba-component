...
public class TestCorbaAckArray extends CorbaComponentTest {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(TestCorbaAckArray.class);
	...
	public void testAckOperation() {
		try {
			...
			ackList.add("874124861");
			ackList.add("874124862");
			ackList.add("874124863");

			headers.put("alarmIds",ackList);
			headers.put("strAckStatus",AckStatus.ACKNOWLEDGE.toString());
			ex.getIn().setHeaders(headers);
			LOGGER.debug("Seding Ack Request");
			producer.process(ex);
			Assert.assertEquals("ackOperator", node1.getAckUserID());
			Assert.assertEquals(4, node1.getAlarm_information_id_list().length);
			Assert.assertEquals("874124860", node1.getAlarm_information_id_list()[0]);
			Assert.assertEquals("874124861", node1.getAlarm_information_id_list()[1]);
			Assert.assertEquals("874124862", node1.getAlarm_information_id_list()[2]);
			Assert.assertEquals("874124863", node1.getAlarm_information_id_list()[3]);
